"""
CNN-LSTM baseline (no attention) for regime-shift detection.

Isolates the CNN's contribution combined with the LSTM's, without the
attention mechanism, as the final piece of the ablation matrix. Reuses
the CNNFeatureExtractor from the main architecture for a fair,
component-for-component comparison, but takes the LSTM's final hidden
state directly for classification rather than an attention-weighted
context vector.
"""
...
from models.architecture import CNNFeatureExtractor
...
class CNNLSTMNoAttention(nn.Module):
    """CNN feature extraction -> LSTM -> final hidden state -> classifier. No attention."""
...
def train(window_size: int = 60, horizon: int = 10, batch_size: int = 32, n_epochs: int = 150, lr: float = 3e-4):
    ...
    print(f"Pooling {len(ALL_CSVS)} tickers for training (CNN-LSTM, no attention, baseline)")
    ...
    model = CNNLSTMNoAttention(input_channels=n_features).to(device)
    ...
    if best_model_state is not None:
        ...
        torch.save(model.state_dict(), "cnn_lstm_baseline_checkpoint.pt")
        print(f"\nLoaded best checkpoint (val_f1={best_f1:.3f}), saved to cnn_lstm_baseline_checkpoint.pt")
